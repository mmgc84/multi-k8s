...
kubectl apply -f k8s
kubectl set image deployments/client-deployment server=mmgc84/multi-client:$SHA
kubectl set image deployments/server-deployment client=mmgc84/multi-server:$SHA
